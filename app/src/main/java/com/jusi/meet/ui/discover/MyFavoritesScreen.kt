...
fun MyFavoritesScreen(
    ...
    val viewModel: MyWorksViewModel = viewModel(
        factory = MyWorksViewModel.factory(app, MyWorksViewModel.Source.MY_FAVORITES)
    )
    val state by viewModel.state.collectAsStateWithLifecycle()

    var pendingUnfavoriteId by remember { mutableStateOf<String?>(null) }

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(stringResource(R.string.profile_my_favorites)) },
                navigationIcon = {
                    ...
                    items(state.posts, key = { it.id }) { post ->
                        PostCard(
                            ...
                            modifier = Modifier.combinedClickable(
                                onClick = { onPostClick(post.id) },
                                onLongClick = { pendingUnfavoriteId = post.id },
                            ),
                        )
                    }
                    if (state.loading) {
    ...
    pendingUnfavoriteId?.let { id ->
        AlertDialog(
            ...
            title = { Text(stringResource(R.string.post_unfavorite)) },
            confirmButton = {
                TextButton(onClick = {
